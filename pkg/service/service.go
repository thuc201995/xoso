...
type XosoService interface {
	...
	GetXSMBByDate(ctx context.Context, date string) (rs []XSMB, err error)
	GetMBByProvince(ctx context.Context, date string, province string) (rs XSMB, err error)
}
...
func (b *basicXosoService) GetXSMBByDate(ctx context.Context, date string) (rs []XSMB, err error) {
	var t time.Time
	client, err := db.GetMongoClient()
	if err != nil {
		return rs, err
	}
	collection := client.Database("test").Collection("xsmb")
	t, err = time.Parse("02-01-2006", date)
	if err != nil {
		return rs, config.DateTimeError
	}
...
func (b *basicXosoService) GetMBByProvince(ctx context.Context, date string, province string) (rs XSMB, err error) {
	var t time.Time
	client, err := db.GetMongoClient()
	if err != nil {
		return rs, err
